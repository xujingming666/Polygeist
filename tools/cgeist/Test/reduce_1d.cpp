...
void kernel_deriche(__ddr float *lhs, __ddr float *rhs, __ddr __output float *out, __ddr __output float *out1, int length, int j, int h) {
    ...
    auto out1_tensor = to_tensor(out1, {arr[0]});

    auto lhs_um = mac_load(lhs_tensor, {vlen}, {0});
    auto rhs_um = mac_load(rhs_tensor, {vlen}, {0});
    auto weight_um = mac_load(rhs_tensor, {vlen}, {0});
    auto out_um = mac_load(out_tensor, {1024}, {0});
    auto out1_um = mac_load(out1_tensor, {1}, {0});
    ...
    for (int i =0, j = 32;i<length; i += 32) {
        ...
        out1_um = mac_min(out_um, 0);
        j+=16;
    }
    mac_store(out_tensor, out_um, {0});
    mac_store(out1_tensor, out1_um, {0});
}
