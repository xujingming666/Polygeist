...
void kernel_deriche(__ddr float *lhs, __ddr float *rhs, __ddr __output float *out, __ddr __output float *out1, int length, int j, int h) {
    ...
    auto out1_tensor = to_tensor(out1, {arr[0], arr[2], arr[2]});
    ...
    auto out_um = mac_load(out_tensor, {1024, 1024, 1024, 1024}, {0, 0, 0, 0});
    auto out1_um = mac_load(out1_tensor, {1024, 1024, 1024}, {0, 0, 0});
    ...
    for (int i =0, j = 32;i<length; i += 32) {
        ...
        out1_um += mac_min(out_um, 1);
        out1_um += mac_add(out1_um, out1_um);
        j+=16;
    }
    mac_store(out_tensor, out_um, {0, 0, 0, 0});
    mac_store(out1_tensor, out1_um, {0, 0, 0});
}
