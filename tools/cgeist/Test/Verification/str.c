...
// CHECK-NEXT:     %[[V3:.+]] = llvm.mlir.addressof
// @str1 : !llvm.ptr<array<11 x i8>> 
// CHECK-NEXT:     %[[V0:.+]] = llvm.mlir.addressof @str0 :
// !llvm.ptr<array<4 x i8>> 
// CHECK-NEXT:     %[[V1:.+]] = "polygeist.pointer2memref"(%[[V0]])
// : (!llvm.ptr<array<4 x i8>>) -> memref<?xi8> 
// CHECK-NEXT:     %[[V2:.+]] = call
// @foo(%[[V1]]) : (memref<?xi8>) -> i32 
// CHECK-NEXT:     %[[V4:.+]] =
