...
static std::string getNSName(const clang::DeclContext *ctx) {
  SmallVector<StringRef, 4> nss;
  auto p = ctx;
  while(p) {
    if (auto ns = dyn_cast<clang::NamespaceDecl>(p)) {
      nss.push_back(ns->getName());
    } else if (auto ns = dyn_cast<clang::RecordDecl>(p)) {
      nss.push_back(ns->getName());
    }
    p = p->getParent();
  }
...
static bool isTensorType(const clang::RecordType & type) {
  auto ns = getNSName(type.getDecl()->getParent());
  return ns == "::" && (type.getDecl()->getName() == "SL_TENSOR" || 
                        type.getDecl()->getName() == "SL_TENSOR_UM");
}

static bool isVectorType(const clang::RecordType & type) {
  auto ns = getNSName(type.getDecl()->getParent());
  return ns == "::" && (type.getDecl()->getName() == "SL_VECTOR");
}

static int getTensorMemorySpace(const clang::RecordType & type){
  if (type.getDecl()->getName() == "SL_TENSOR")
    return 1;
  return 0;
}

static bool isMlirTensorType(const mlir::Type type) {
...
mlir::Value MLIRScanner::getComplexPart(mlir::Location loc, mlir::Value complex,
  ...
  if (auto mt = dyn_cast<mlir::MemRefType>(ref.getType())) {
    ...
    return builder.create<mlir::LLVM::LoadOp>(loc, getValuePtrType(ref), ref);
  } else {
    assert(0);
  }
}
...
void MLIRScanner::init(mlir::func::FuncOp function, const FunctionDecl *fd) {
  ...
  llvm::SmallVector<clang::ParmVarDecl *> outputArgs;
  ...
  if (function.getFunctionType().getResults().size() > outputArgs.size()) {
    ...
    if (D->getParent()->isUnion() && D->isDefaulted()) {
      ...
      if (auto MT = dyn_cast<MemRefType>(V.getType())) {
        V = builder.create<polygeist::Memref2PointerOp>(
            loc, LLVM::LLVMPointerType::get(builder.getContext()), V);
      }
      ...
      if (auto MT = dyn_cast<MemRefType>(src.getType())) {
        src = builder.create<polygeist::Memref2PointerOp>(
            loc, LLVM::LLVMPointerType::get(builder.getContext()), src);
      }
  ...
  if (function.getFunctionType().getResults().size()) {
    llvm::SmallVector<mlir::Value> retValues;

    int idxOffset = 0;
    if (function.getFunctionType().getResults().size() > outputArgs.size()) {
      mlir::Value vals[1] = {
        builder.create<mlir::memref::LoadOp>(loc, returnVal)};
      retValues.push_back(vals[0]);
      idxOffset = 1;
    }
    ...
    builder.create<func::ReturnOp>(loc, retValues);
  } else
    builder.create<func::ReturnOp>(loc);
    ...
    if (LLVMABI) {
      if (name)
        if (auto var = dyn_cast<VariableArrayType>(
          ...
          alloc = builder.create<mlir::LLVM::AllocaOp>(
              varLoc, LLVM::LLVMPointerType::get(builder.getContext(), memspace), t,  len);
      ...
      if (!alloc) {
        alloc = abuilder.create<mlir::LLVM::AllocaOp>(
            varLoc, mlir::LLVM::LLVMPointerType::get(abuilder.getContext(), memspace), t, 
      ...
      if (memspace != 0) {
        alloc = abuilder.create<polygeist::Pointer2MemrefOp>(
            ...
            abuilder.create<polygeist::Memref2PointerOp>(
                varLoc, LLVM::LLVMPointerType::get(abuilder.getContext(), 0), alloc));
      }
      ...
      if (auto var = dyn_cast<VariableArrayType>(
        ...
        if (memspace != 0) {
          alloc = abuilder.create<polygeist::Pointer2MemrefOp>(
              ...
              abuilder.create<polygeist::Memref2PointerOp>(
                  varLoc, LLVM::LLVMPointerType::get(abuilder.getContext(), 0),
                  alloc));
        }
      }
    ...
    if (InitListExpr *initListExpr = dyn_cast<InitListExpr>(expr)) {
      ...
      if (initListExpr->hasArrayFiller()) {
        if (auto MT = dyn_cast<MemRefType>(toInit.getType())) {
          ...
          if (auto AT = dyn_cast<LLVM::LLVMArrayType>(getValuePtrType(toInit))) {
            num = AT.getNumElements();
          } else if (auto AT =
                         dyn_cast<LLVM::LLVMStructType>(getValuePtrType(toInit))) {
            num = AT.getBody().size();
          } else {
            toInit.getType().dump();
            llvm_unreachable(
        ...
        if (shape.size() == 0) {
          ...
          if (ET.isa<mlir::LLVM::LLVMStructType>() ||
            ...
            toInit = builder.create<polygeist::Memref2PointerOp>(
                loc, LLVM::LLVMPointerType::get(builder.getContext(), mt.getMemorySpaceAsInt()),
                toInit);
      ...
      for (unsigned i = 0, e = num; i < e; ++i) {
        ...
        if (auto mt = dyn_cast<MemRefType>(toInit.getType())) {
          ...
          auto ET = getValuePtrType(toInit);
          mlir::Type nextType;
...
ValueCategory MLIRScanner::VisitVarDecl(clang::VarDecl *decl) {
  ...
  if (isMlirTensorType(subType)) {
    if (auto init = decl->getInit()) {
      ...
      llvm_unreachable("tensor expr do not have init expr \n");
  }
...
ValueCategory MLIRScanner::VisitConstructCommon(clang::CXXConstructExpr *cons,
  ...
  if (isa<mlir::RankedTensorType>(subType)) {

    if (decl->isTrivial() && decl->isDefaultConstructor()) {
      assert(false && " default constructor withoud parameters is not supported \n ");
    }
    
    unsigned typeQuals = 0;
    if (decl->isTrivial() && decl->isCopyConstructor(typeQuals))
      return Visit(cons->getArg(0));
...
ValueCategory MLIRScanner::CommonArrayToPointer(mlir::Location loc,
  ...
  if (auto PT = dyn_cast<mlir::LLVM::LLVMPointerType>(scalar.val.getType())) {
    if (getValuePtrType(scalar.val).isa<mlir::LLVM::LLVMPointerType>())
      return ValueCategory(scalar.val, /*isRef*/ false);
    mlir::Value vec[2] = {builder.create<ConstantIntOp>(loc, 0, 32),
                          builder.create<ConstantIntOp>(loc, 0, 32)};
    if (!getValuePtrType(scalar.val).isa<mlir::LLVM::LLVMArrayType>()) {
      EmittingFunctionDecl->dump();
      function.dump();
...
ValueCategory MLIRScanner::CommonArrayLookup(mlir::Location loc,
  ...
  if (val.getType().isa<LLVM::LLVMPointerType>()) {
    ...
    // TODO sub
    return ValueCategory(
        builder.create<mlir::LLVM::GEPOp>(loc, val.getType(), getValuePtrType(val), val, vals),
        /*isReference*/ true);
  }
  if (!val.getType().isa<MemRefType>()) {
    EmittingFunctionDecl->dump();
...
ValueDecl * MLIRScanner::getRelVarDecl(clang::Expr *expr) {
  for (clang::Stmt* child : expr->children()) {
    if (auto declRef = dyn_cast<clang::DeclRefExpr>(child)) {
      return declRef->getDecl();
    }
    if (isa<clang::Expr>(child))
      return getRelVarDecl(dyn_cast<clang::Expr>(child));
  }
  return nullptr;
}
          ...
          if (auto omt = dyn_cast<MemRefType>(dst.getType())) {
            if (auto mt = dyn_cast<MemRefType>(omt.getElementType())) {
              ...
              allocv = builder.create<polygeist::Memref2PointerOp>(
                  loc, LLVM::LLVMPointerType::get(builder.getContext(), 1),
                  allocv);
              allocv =
...
ValueCategory MLIRScanner::VisitUnaryOperator(clang::UnaryOperator *U) {
  ...
  case clang::UnaryOperator::Opcode::UO_PostInc: {
    ...
    if (auto ft = dyn_cast<mlir::FloatType>(ty)) {
      ...
      next = builder.create<LLVM::GEPOp>(
          loc, pt, getValuePtrType(prev), prev,
...
ValueCategory MLIRScanner::VisitTensorBinaryOperator(clang::BinaryOperator *BO) {
...
ValueCategory MLIRScanner::VisitBinaryOperator(clang::BinaryOperator *BO) {
  ...
  if (isa<mlir::RankedTensorType>(retType)) {
    return VisitTensorBinaryOperator(BO);
  }
  ...
  case clang::BinaryOperator::Opcode::BO_NE: {
    ...
    if (auto mt = dyn_cast<mlir::MemRefType>(rhs_v.getType())) {
      rhs_v = builder.create<polygeist::Memref2PointerOp>(
          loc, LLVM::LLVMPointerType::get(builder.getContext()), rhs_v);
    }
    mlir::Value res;
    if (lhs_v.getType().isa<mlir::FloatType>()) {
  ...
  case clang::BinaryOperator::Opcode::BO_Add: {
    ...
    if (lhs_v.getType().isa<mlir::FloatType>()) {
      ...
      return ValueCategory(
          builder.create<LLVM::GEPOp>(loc, pt, getValuePtrType(lhs_v), lhs_v,
  ...
  case clang::BinaryOperator::Opcode::BO_Assign: {
    ...
    if (auto PT = dyn_cast<mlir::LLVM::LLVMPointerType>(lhs.val.getType()))
      subType = getValuePtrType(lhs.val);
    else
...
ValueCategory MLIRScanner::CommonFieldLookup(mlir::Location loc,
  ...
  if (auto mt = dyn_cast<MemRefType>(val.getType())) {
    ...
    assert(ET.isa<mlir::LLVM::LLVMStructType>() ||
           ET.isa<mlir::LLVM::LLVMArrayType>());
    val = builder.create<polygeist::Memref2PointerOp>(
        loc, LLVM::LLVMPointerType::get(builder.getContext(), mt.getMemorySpaceAsInt()), val);
  }
...
ValueCategory MLIRScanner::VisitDeclRefExpr(DeclRefExpr *E) {
  ...
  if (auto tocall = dyn_cast<FunctionDecl>(E->getDecl())) {
    ...
    return ValueCategory(builder.create<polygeist::GetFuncOp>(
                             loc, LLVM::LLVMPointerType::get(builder.getContext()), f.getName()),
                         /*isReference*/ true);
  }
  ...
  if (auto PD = dyn_cast<VarDecl>(E->getDecl())) {
    ...
    if (isa<mlir::RankedTensorType>(ty)) {
      auto res = getLocalValue(PD);
      assert(res.val);
      return res;
    }
  ...
  for (auto tup : llvm::zip(BaseTypes, BaseVirtuals)) {
    ...
    if (subIndex) {
      ...
      if (!done) {
        ...
        mlir::Type ET;
        if (auto ST =
                dyn_cast<mlir::LLVM::LLVMStructType>(getValuePtrType(value))) {
          ET = ST.getBody()[fnum];
        } else {
          ET = getValuePtrType(value)
                   .cast<mlir::LLVM::LLVMArrayType>()
                   .getElementType();
        }
...
ValueCategory MLIRScanner::VisitCastExpr(CastExpr *E) {
  ...
  case clang::CastKind::CK_DerivedToBase: {
    ...
    if (E->getCastKind() != clang::CastKind::CK_UncheckedDerivedToBase &&
      ...
      if (auto MT = dyn_cast<MemRefType>(ptr.getType()))
        ptr = builder.create<polygeist::Memref2PointerOp>(
            loc, LLVM::LLVMPointerType::get(builder.getContext()), ptr);
  ...
  case clang::CastKind::CK_LValueBitCast: {
    ...
    if (auto spt = dyn_cast<mlir::LLVM::LLVMPointerType>(scalar.getType())) {
      ...
      pt = LLVM::LLVMPointerType::get(builder.getContext(),
                                      spt.getAddressSpace());
  ...
  case clang::CastKind::CK_PointerToBoolean: {
    ...
    if (auto mt = dyn_cast<mlir::MemRefType>(scalar.getType())) {
      scalar = builder.create<polygeist::Memref2PointerOp>(
          loc, LLVM::LLVMPointerType::get(builder.getContext()), scalar);
    }
...
MLIRASTConsumer::GetOrCreateMLIRFunction(const FunctionDecl *FD,
  ...
  SmallVector<bool, 4> outputFlags;
  for (auto parm : FD->parameters()) {
